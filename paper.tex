...
as predictors of $\Delta W$, yet only the fine structure serves as a
causal handle: the generator bias of Axis~4 is conditioned on
exposed-site geometry, not on the aggregate $[W_0, B_\mathrm{mag}]$.
Two descriptions with equivalent predictive power differ entirely in
their causal leverage within the same system at the same temperature.
\end{abstract}
...
Five values of $\varepsilon$ are tested: $\varepsilon=0$ (null control, gap should be near zero with CI spanning zero) and $\varepsilon\in\{0.02,0.05,0.10,0.20\}$ (active perturbations).
...
The crossover is not specific to $E$: the champion organism $E_{\geq 3}+J$ also reverses sign between regimes, confirming the crossover is a regime property, not an artefact of the specific fine predictor used for Axis~A.
...
The aligned and misaligned gaps are not equal in magnitude: at $\varepsilon=0.10$ the aligned gap is $+\AxisBAlignedTen$ while the misaligned gap is $\AxisBMisalnTen$, a ratio of approximately $\AxisBAsymRatio\times$. This asymmetry has a structural explanation. Fully exposed sites at $T=\IsingTOrd$ flip with probability $P\approx 0.988$ under the unperturbed Glauber kernel; the aligned arm can add at most $\min(1, 0.988+\varepsilon)-0.988 \approx \varepsilon_{\rm eff}\ll\varepsilon$ (ceiling effect), while the misaligned arm subtracts the full $\varepsilon$ (no floor reached). The misaligned arm therefore perturbs the dynamics more strongly at the same nominal $\varepsilon$, producing a larger gap magnitude. This asymmetry is expected and does not undermine the sign-flip discriminator; it does mean the experiment is a stronger test of suppression than of promotion at these temperatures.
...
structural class is necessary at all, compared with targeting random sites.
This question was \emph{not} addressed by the companion probe and remains
open; the present results establish the sign-flip for $d_c=4$ targeting
but do not test the counterfactual of random-site targeting.
...
Third, the causal claim is class-level, not individual-level: the
direction-dominance result (Section~\ref{sec:causal_protocol}) shows that
within the $d_c=4$ class, individual site identity is inert; whether
class-level targeting ($d_c=4$ versus random sites) is itself necessary
remains an open question not addressed here.
...
Importantly, the causal leverage demonstrated here is class-level:
the intervention is conditioned on membership in the $d_c=4$ class, and
the direction-dominance result (Section~\ref{sec:causal_protocol}) shows
that individual site identity within that class is inert.
The fine description $E_{\geq 3}+J$ is necessary for class identification,
not for within-class individualisation.
A scientist selecting an intervention target based on predictive accuracy
alone could not identify the $d_c=4$ class and would lose the ability
to perform the direction-specific control demonstrated in Axis~4.

\subsection{Cross-system comparison}

The distinction between site-selection and flip-direction causal leverage
may not be universal.
In a preliminary companion cellular-automaton (CA) study (to be reported
separately), site-selection \emph{anti-alignment}---biasing transitions
\emph{away} from the structure-predicted target---produced a consistent
effect across all 12 tested conditions, suggesting that site identity
may be the operative causal handle in that system.
If confirmed, the Ising and CA results would be qualitatively different:
in Ising, causal power is direction-dominated because exposed sites are
already near the flip-probability ceiling; in the CA, causal power would
be site-selection dominated because site identity determines transition
eligibility in a way that is not saturated.
We flag this contrast as a suggestive preliminary finding, not a
confirmed cross-system result; independent replication and fuller
reporting are needed before strong conclusions can be drawn.
...
Three specific gaps in the present study should be addressed in future work.
First, only two temperatures are tested ($T=\IsingTOrd$ and $T=\IsingTDis$),
one on each side of $T_c \approx 2.269$.
The crossover structure near criticality is unresolved; a third temperature
near $T_c$ (e.g.\ $T \approx 2.20$) would determine whether the fine
advantage degrades smoothly or abruptly and would strengthen the crossover
claim.
Second, all results use a fixed horizon $K=\IsingK$ sweeps.
Horizon sensitivity ($K \in \{10, 50\}$) is untested; as the characteristic
coarsening timescale varies with temperature, $K=20$ may correspond to
qualitatively different phases of evolution at the two anchor temperatures.
Third, the class-level causal claim---that $d_c=4$ site targeting is
necessary---is assumed, not tested.
A direct comparison of $d_c=4$-targeted arms against random-site-targeted
arms at the same $\varepsilon$ would determine whether the structural class
itself is causally privileged or whether direction alone suffices regardless
of which sites are targeted.
...
zero), yet only the fine structure serves as a direction-specific causal
handle.
