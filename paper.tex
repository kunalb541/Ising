...
those flips retards it; this direction-specific sign flip holds at all
active perturbation strengths (\AxisBSignFlips{} sign flips on non-null
$\varepsilon$ values), and \emph{also holds in the disordered phase}
($T=\IsingTDis$, \CausalDisSignFlips{} sign flips), where the predictive
advantage of $E$ has reversed.
A class-level control (budget-matched random-site arm with the same
directional logic) and a filter-decomposition test together show that
the structural and random arms are two distinct per-site interventions
on the same generator, neither reducible to a single-class mechanism;
both, however, require local information ($s\cdot h$) that global
aggregate descriptions cannot supply.
...
The straightforward reading of this decomposition is that the random
arm's effect is \emph{not} concentrated at $d_c=0$ (a pure
bulk-suppression mechanism) and \emph{not} concentrated at $d_c\geq 3$
(a high-exposure subset that mimics the structural arm at lower
budget); it is distributed across the intermediate $d_c\in\{1,2\}$
classes that neither filter retains.
We therefore do not interpret the random arm as ``bulk suppression'';
it is a distributed-class intervention with no single-class mechanism.
This falsifies the simplest mechanism story we could have told for the
random arm.

\paragraph{What the controls do and do not establish.}
The structural arm earns its sign-flip support \AxisBSignFlips{} via a
clearly localised mechanism (promotion at $d_c=4$ sites, ceiling-limited).
The random arm earns its sign-flip support via a distributed mechanism
that we have characterised only by elimination: not the bulk subset, not
the high-exposure subset alone.
...
achieves comparable or larger positive gaps; the filter decomposition
(Section~\ref{sec:filter_decomp}) shows the random-arm effect is not
attributable to any single $d_c$ class.
We therefore do not claim $d_c=4$ class identification is uniquely
necessary; we claim that both interventions condition on the per-site
quantity $s\cdot h$, which $[W_0, B_{\rm mag}]$ does not encode.
...
sign-flip on future $\Delta W$ in the ordered phase, and
\CausalDisSignFlips{} in the disordered phase as well
(Section~\ref{sec:causal_dis}).
The causal handle does not inherit the predictive sign reversal of
Axis~2: per-site directional intervention at $d_c=4$ sites works
across both phases.
...
finite-size stability of $\Delta R^2$ across $L\in\{32,64,128\}$
(though the fitted Ridge weights themselves are $L$-specific),
horizon robustness across $K\in\{10,20,50\}$ (fine advantage increases
weakly with $K$), and a smooth $E$ vs $\phi_{\rm var8}$ crossover
through criticality ($T\in\{1.80,2.00,2.20,2.35,2.50\}$ traces
$+0.18\to+0.11\to0\to0\to-0.07$).
