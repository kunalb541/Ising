...
%-----------------------------------------------------------------------
\begin{abstract}
We ask whether predictive sufficiency implies causal access in the two-dimensional kinetic Ising model, and whether predictive ranking is stable across dynamical regimes. The answer to both is no. The central result is a predictor-handle dissociation: a coarse macro-state and the fine champion are statistically indistinguishable as predictors of future domain-wall change, yet only the fine structure yields a direction-specific causal handle. A regime-crossover result earns the regime-dependence qualifier and blocks the objection that fine descriptions win simply by having more bits. Four empirical axes support these claims.
(\textbf{Axis~1, organism comparison})\;
Against a block-variance coarse rival that achieves $R^2 \approx 0$ for
$\Delta W$ in the ordered phase (a near-null baseline), the fine champion
$E_{\geq 3}+J$ achieves $\Delta R^2 = +\AxisOneDR$
(95\% CI $[+\AxisOneDRlo,\,+\AxisOneDRhi]$).
Against the stronger coarse rival $[W_0, B_\mathrm{mag}]$, the gap
narrows to $\Delta R^2 = \AxisOneVsWB$ (CI includes zero): the two
descriptions are statistically indistinguishable as predictors.
...
(\textbf{Axis~2, regime crossover})\; includes a near-critical probe at
$T=\NearTcTemp$, which finds $\Delta R^2\approx\NearTcDR$ (CI spans zero),
placing the crossover between $T=\IsingTOrd$ and $T=\NearTcTemp$.
(\textbf{Axis~4, causal control})\; Biasing the Glauber generator toward
wall-reducing flips at exposed sites accelerates future wall removal
(aligned arm removes more walls than baseline), while biasing against
those flips retards it; this direction-specific sign flip holds across
all active perturbation strengths (\AxisBSignFlips{} sign flips on non-null $\varepsilon$ values).
A class-level control (budget-matched random-site arm with the same directional
logic) shows that the operative causal mechanism is the per-site
directional conditioning---not $d_c=4$ class identity specifically---and
that global aggregate descriptions cannot provide this per-site handle.
(\textbf{Dissociation})\; The aggregate coarse macro-state
$[W_0, B_\mathrm{mag}]$ ($R^2 = \AxisCRWB$) and the fine champion
$E_{\geq 3}+J$ ($R^2 \approx \AxisOneDR$) are statistically indistinguishable
as predictors of $\Delta W$, yet only per-site fine structure enables
a direction-specific causal handle: global aggregate descriptions carry
no per-site directional information and cannot be used to implement
the generator bias of Axis~4.
Two descriptions with equivalent predictive power differ entirely in
their capacity to support per-site causal intervention.
\end{abstract}
...
The central question is whether predictive sufficiency implies causal
access: can a description that accurately forecasts future domain-wall
change be used to controllably change it?
We find the answer is no, and that the dissociation is not an artefact of
a particular baseline choice: even the strongest available coarse rival
$[W_0, B_\mathrm{mag}]$ matches the fine champion predictively
($\Delta R^2 = \AxisOneVsWB$, CI includes zero), yet only the fine
structure provides a direction-specific causal handle.
The crossover result earns the regime-dependence qualifier: the
fine-over-coarse predictive advantage reverses sign from ordered
($+\AxisAOrdDR$) to disordered ($\AxisADisDR$ phase), blocking the
objection that fine descriptions simply win by having more bits.
The fine champion is $E_{\geq 3}+J$; the target most specifically
favoured by fine descriptions is $\Delta W$; and the causal sign-flip
holds across all \AxisBSignFlips{} active perturbation strengths
...
This is not a failure of the fine description; it is the basis of the dissociation result (Section~\ref{sec:diss}): two predictors with equivalent forecasting power differ entirely in their causal leverage.
...
To probe the crossover structure near criticality, we additionally ran
$N=1000$ configurations at $T=\NearTcTemp$ (EQUIL $=2000$
sweeps for the longer correlation length near $T_c$).
The result:
\begin{equation}
\Delta R^2(E\text{ vs }\phi_{\rm var8})\big|_{T=\NearTcTemp}
= \NearTcDR
\quad\text{(95\% CI }[\NearTcDRlo,\,+\NearTcDRhi]\text{)}.
\end{equation}
The CI spans zero, placing $T=\NearTcTemp$ in the indistinguishable
transition band: $+\AxisAOrdDR$ (ordered), $\approx 0$ (near $T_c$),
$\AxisADisDR$ (disordered).
The crossover therefore occurs between $T=\IsingTOrd$ and $T=\NearTcTemp$,
and the transition from fine-wins to fine-indistinguishable appears
abrupt rather than gradual---consistent with the rapid breakdown of
the sparse-interface geometry as $T\to T_c$.
...
The result is \AxisBSupport{}: \AxisBSignFlips{} active $\varepsilon$ values pass the
sign-flip discriminator (the $\varepsilon=0$ null produces a gap consistent with zero as expected).
...
arm consistently removes fewer (gap: $\AxisBMisalnTen$, CI excludes zero).
...
Fully exposed sites at $T=\IsingTOrd$ flip with probability $P\approx 0.988$
under the unperturbed Glauber kernel; the aligned arm can add at most
$\varepsilon_{\rm eff} = 1-0.988 = 0.012$ (ceiling constraint), while
the misaligned arm subtracts the full $\varepsilon$.
The predicted ratio at $\varepsilon=0.10$ is $\varepsilon/\varepsilon_{\rm eff}=0.10/0.012\approx 8.3\times$, close to the observed $\AxisBAsymRatio\times$.
...
$\delta W = -4$ upon flipping and has $P_{\rm flip}\approx 0.988$.
...
at $P_{\rm flip}\approx 0.988$, so biasing one individual versus another
...
The structural arm consistently achieves a larger positive gap;
the random arm gap is near zero or negative (see text), confirming
that $d_c=4$ class-level identification is causally necessary.}
...
The structural arm is ceiling-limited at $T=\IsingTOrd$: $d_c=4$ sites
already flip with $P\approx 0.988$, so the aligned bias adds at most
$\varepsilon_{\rm eff}\approx 0.012$ effective probability ($1-0.988$
before hitting the ceiling at $P=1$).
The random arm, by contrast, selects sites with probability $\rho = E_0/L^2\approx
\MeanELSixtyFour/4096\approx 1\%$; approximately $99\%$ of selected
sites have $d_c\leq 1$ (domain interior, $s\cdot h > 0$), so the aligned
logic there \emph{suppresses} wall-creating flip attempts
($P\approx 0.012 \to \max(0,\, 0.012-\varepsilon)=0$ at $\varepsilon=0.10$).
Eliminating these bulk fluctuations accelerates net coarsening
through a different mechanism: not by promoting specific wall-reducing
events at geometrically special sites, but by broadly suppressing
wall-creating fluctuations at randomly selected domain-interior sites.

The two arms therefore represent \emph{qualitatively different} causal mechanisms:
the structural arm (\emph{promote} coarsening-relevant events at $d_c=4$ sites)
and the random arm (\emph{suppress} wall-creating fluctuations at random sites).
Both achieve positive gaps, but the structural arm saturates with $\varepsilon$
(ceiling-limited) while the random arm grows monotonically (not floor-limited
at these $\varepsilon$ values).

The conclusion from this control is not that $d_c=4$ class targeting is
causally privileged in magnitude, but that the \emph{directional logic}---
conditioning the generator bias on whether a flip is wall-reducing or
wall-increasing---is the operative causal mechanism.
This directional logic requires per-site local state information
(the sign of $s\cdot h$); it cannot be implemented from the global
aggregate $[W_0, B_{\rm mag}]$, which carries no per-site information.
The predictor-handle dissociation therefore remains: two descriptions
equivalent in predictive accuracy differ in their capacity to provide
per-site directional causal handles; global aggregate descriptions
cannot serve this role regardless of their predictive power.

\subsection{Predictor-handle dissociation}
\label{sec:diss}

Two descriptions of the same configuration predict $\Delta W$ with
statistically indistinguishable accuracy:
\begin{equation}
R^2([W_0, B_\mathrm{mag}]) = \AxisCRWB, \qquad
R^2(E_{\geq 3}+J) \approx \AxisOneDR, \qquad
\Delta R^2 = \AxisOneVsWB \ \ \text{(CI includes zero)}.
\end{equation}
Both also outperform the single-feature count $R^2(E) = \AxisCRE$ by a
large margin ($+\AxisCGap$).
The key asymmetry is causal, not predictive: the generator bias of Axis~4
is conditioned on the local exposed-site geometry that $E_{\geq 3}+J$
encodes, and produces a direction-specific sign flip ($\AxisBSignFlips$).
The aggregate $[W_0, B_\mathrm{mag}]$ carries no site-specific structural
information and cannot be used as a generator-level causal handle.
\textbf{Two descriptions with equivalent predictive power over $\Delta W$
differ entirely in their causal leverage within the same system at the
same temperature.}
...
statistical forecasting task, but it does not identify the structural
class of sites ($d_c=4$) that drives coarsening.
The fine structure identifies that class, and that is precisely what
...
Third, the class-level control (Section~\ref{sec:axis4_random}) shows
that budget-matched random-site targeting with the same directional logic
achieves comparable or larger positive gaps, mediated through
suppression of bulk fluctuations rather than promotion of specific
coarsening events; this reveals that the directional logic (per-site
wall-reducing vs wall-increasing) is the operative mechanism, not
$d_c=4$ class identification per se.
...
specific, rather than a generic perturbation effect \cite{woodward2003,pearl2009}.
...
Yet only $E_{\geq 3}+J$---or more precisely, the exposed-site geometry
it encodes---can be used to construct a direction-specific causal
handle: the generator bias of Axis~4 acts on the locally identified
$d_c=4$ structural class, which $[W_0, B_{\rm mag}]$ does not resolve.
The two descriptions encode complementary information:
$W_0$ summarises \emph{how much} coarsening will occur on average
(aggregate potential), while exposed-site geometry identifies
\emph{where} the structurally relevant class of sites is located
(class-level localisation).
Equal predictive power masks a qualitative difference in causal
accessibility.

The class-level control (Section~\ref{sec:axis4_random}) refines this
picture: random-site targeting with the same directional logic achieves
comparable or larger gaps by suppressing bulk fluctuations, revealing
that the operative causal mechanism is per-site directional conditioning
rather than $d_c=4$ class identity per se.
What the fine description $E_{\geq 3}+J$ provides is the ability to
identify $d_c=4$ sites and use them as specifically wall-reducing
targets; the dissociation persists at the level of per-site structural
vs global aggregate descriptions, not at the level of $d_c=4$ vs
any per-site operation.
...
Remaining open questions: whether the two identified causal mechanisms
($d_c=4$-specific promotion vs bulk-suppression via random selection) differ
in efficiency under a fixed total perturbation budget; how the crossover
structure between $T=\IsingTOrd$ and $T=\NearTcTemp$ resolves at
intermediate temperatures; and how the horizon-monotone increase of the
...
$[W_0, B_\mathrm{mag}]$ ($R^2 = \AxisCRWB$) and $E_{\geq 3}+J$
($R^2 \approx \AxisOneDR$) are statistically indistinguishable as
predictors of $\Delta W$ ($\Delta R^2 = \AxisOneVsWB$, 95\% CI includes
zero), yet only per-site fine structure enables a direction-specific causal
handle: global aggregate descriptions cannot supply the per-site directional
information required for the generator bias of Axis~4.
The best predictor and the best causal handle are different objects.
\end{enumerate}

Additional robustness results (Section~\ref{sec:robustness}) confirm
finite-size stability across $L\in\{32,64,128\}$, horizon robustness
across $K\in\{10,20,50\}$ (fine advantage increases weakly with $K$),
and near-critical structure ($\Delta R^2\approx 0$ at $T=\NearTcTemp$,
indicating an abrupt crossover near $T_c$).

In this system, the relative usefulness of fine and coarse descriptions
depends on the target, the regime, and the type of causal access sought.
