...
(95\% CI $[+\AxisOneDRlo,\,+\AxisOneDRhi]$) against this rival,
which achieves $R^2 \approx 0$ for $\Delta W$ at this temperature.
...
\textit{Reproducibility note:} Stochastic dynamics are implemented via Numba-compiled functions; the Numba random-number generator is seeded independently from the worker-level seed to ensure cross-run reproducibility. Reported confidence intervals are stable to this level of control.
...
A companion probe ($N=2000$ worlds, same $L$, $K$, $T$) tested whether
\emph{which sites} receive the bias matters---comparing a site-selection
aligned arm against a site-selection anti-aligned arm at the same nominal
$\varepsilon$.
...
(toward wall-reducing transitions) rather than in site identity per se.
This is consistent with the ceiling effect described above: exposed sites
are already at $P_{\rm flip}\approx 0.988$, so choosing \emph{which}
exposed site to bias has negligible impact compared with choosing
\emph{whether} to increase or decrease the flip probability.
...
handle: the generator bias of Axis~4 acts on locally identified
exposed sites, which $[W_0, B_{\rm mag}]$ does not resolve.
...
\emph{where} the next coarsening events will occur
(structural localisation).
...
alone could not distinguish between these two descriptions---and would
lose the ability to perform the direction-specific control demonstrated
in Axis~4.
...
is not universal.
In a companion cellular-automaton (CA) study, site-selection
\emph{anti-alignment}---biasing transitions \emph{away} from the
structure-predicted target---produced a consistent effect across all
12 tested conditions, indicating that site identity is the operative
causal handle in that system.
The Ising and CA results are therefore qualitatively different:
in Ising, causal power is direction-dominated because exposed sites are
already near the flip-probability ceiling; in the CA, causal power is
site-selection dominated because site identity determines transition
eligibility in a way that is not saturated.
These two systems provide an empirical contrast suggesting that the
dominant form of causal leverage---site-structural identity versus
transition direction---varies with the dynamical regime and the degree
to which flip probabilities are already saturated by the base dynamics.
